...
export function accountInviteEmail({ name, email, activationToken, institutionName }) {
  ...
  const link = `${appUrl}/activate?token=${activationToken}`;
  return {
...
export function preferenceConfirmationEmail({ name, cycleName, preferenceSummary }) {
  ...
  return {
    ...
    html: layout('Preferences Submitted', `
      ...
      ${button(`${appUrl}/member/preferences`, 'View Preferences')}
    `),
    text: `Hello ${name},\n\nYour preferences for ${cycleName} have been saved. You can update them before the deadline at ${appUrl}/member/preferences\n\n— SER-CAT Scheduler`,
  };
}
...
export function deadlineReminderEmail({ name, cycleName, daysRemaining, deadlineDate }) {
  ...
  return {
    ...
    html: layout(`Preferences ${urgency}`, `
      ...
      ${button(`${appUrl}/member/preferences`, 'Submit Preferences')}
      ${p('If you have already submitted, you can ignore this reminder.')}
    `),
    text: `Hello ${name},\n\nPreferences for ${cycleName} are ${urgency} (${deadlineDate}). Submit at ${appUrl}/member/preferences\n\n— SER-CAT Scheduler`,
  };
}
...
export function schedulePublishedEmail({ name, cycleName, assignmentSummary }) {
  ...
  return {
    ...
    html: layout('Schedule Published', `
      ...
      ${button(`${appUrl}/member/schedule`, 'View My Schedule')}
    `),
    text: `Hello ${name},\n\nThe schedule for ${cycleName} has been published. View it at ${appUrl}/member/schedule\n\n— SER-CAT Scheduler`,
  };
}
...
export function swapRequestUpdateEmail({ name, cycleName, status, adminNotes }) {
  ...
  return {
    ...
    html: layout(`Swap Request ${statusLabel}`, `
      ...
      ${button(`${appUrl}/member/shift-changes`, 'View Details')}
    `),
    text: `Hello ${name},\n\nYour swap request for ${cycleName} has been ${statusLabel.toLowerCase()}.${adminNotes ? `\nAdmin notes: ${adminNotes}` : ''}\n\nView details at ${appUrl}/member/shift-changes\n\n— SER-CAT Scheduler`,
  };
}

export function emailVerifyEmail({ name, verifyUrl }) {
  return {
    subject: 'SER-CAT Scheduler — Confirm Your New Email Address',
    html: layout('Confirm Your New Email Address', `
      ${p(`Hello ${name || 'there'},`)}
      ${p('A request was made to change the email address on your SER-CAT Scheduler account. Click below to confirm your new email address. This link expires in 24 hours.')}
      ${button(verifyUrl, 'Confirm New Email')}
      ${p("If the button doesn't work, copy this URL:")}
      <p style="margin:0 0 12px;font-size:12px;color:${BRAND.gray};word-break:break-all;">${verifyUrl}</p>
      ${p('If you did not request this change, you can safely ignore this email. Your current email address will remain unchanged.')}
    `),
    text: `Hello ${name || 'there'},\n\nConfirm your new email address: ${verifyUrl}\n\nThis link expires in 24 hours.\n\nIf you did not request this change, ignore this email.\n\n— SER-CAT Scheduler`,
  };
}
...
export function adminAlertEmail({ subject: alertSubject, message }) {
  ...
  return {
    ...
    html: layout('Admin Alert', `
      ${p(message)}
      ${button(`${appUrl}/admin/dashboard`, 'Open Admin Dashboard')}
    `),
...
export function manualReminderEmail({ name, cycleName, customMessage }) {
  ...
  return {
    ...
    html: layout('Message from SER-CAT Admin', `
      ${p(`Hello ${name},`)}
      ${p(customMessage)}
      ${button(`${appUrl}/member/preferences`, 'Go to Scheduler')}
    `),
    text: `Hello ${name},\n\n${customMessage}\n\n${appUrl}/member/preferences\n\n— SER-CAT Scheduler`,
  };
}
