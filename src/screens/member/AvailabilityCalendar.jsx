    ...
    <div className="space-y-3">
      {adminNotes ? (
        <div className="rounded-xl border px-4 py-3" style={{ background: CONCEPT_THEME.sand, borderColor: CONCEPT_THEME.border }}>
          <p className="mb-1 text-xs font-bold uppercase tracking-wide" style={{ color: CONCEPT_THEME.navy }}>Admin Notice</p>
          <p className="text-sm whitespace-pre-wrap" style={{ color: CONCEPT_THEME.text }}>{adminNotes}</p>
        </div>
      ) : null}

      {timingExceptions.length > 0 ? (
        <div className="rounded-xl border px-4 py-3" style={{ background: CONCEPT_THEME.sand, borderColor: CONCEPT_THEME.border }}>
          <p className="mb-2 text-xs font-bold uppercase tracking-wide" style={{ color: CONCEPT_THEME.navy }}>Shift Timing Changes</p>
          <div className="space-y-1">
            {[...timingExceptions].sort((a, b) => a.date.localeCompare(b.date) || a.shift.localeCompare(b.shift)).map((o) => (
              <p key={`${o.date}-${o.shift}`} className="text-sm" style={{ color: CONCEPT_THEME.text }}>
